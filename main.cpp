...
void init_game_world_data(shared_ptr<game_world_data> world) 
{
	...
	shared_ptr<player> create_player(shared_ptr<settings_manager> settings_admin, shared_ptr<resource_manager> resource_admin)
	{
	...
	shared_ptr<game_object> setup_player(shared_ptr<settings_manager> settings_admin, shared_ptr<resource_manager> resource_admin, shared_ptr<event_manager> event_admin /*,shared_ptr<vector<shared_ptr<square>>> rooms*/)
	{
		...
		the_player-> set_tag(constants::game_world);
		the_player->add_component(room_component);
		return the_player;
	}

vector<shared_ptr<game_object>> load_content(std::shared_ptr<event_manager> event_admin, std::shared_ptr<resource_manager> resource_admin, std::shared_ptr<settings_manager> settings_admin, shared_ptr<game_world> world)
{
	...
	// Create the player
	const auto player = setup_player(settings_admin, resource_admin, event_admin /*, rooms*/);
...
int main(int argc, char *args[])
{
	try
	{
		...
		const auto is_loaded = succeeded(structure->load_content(), "loading content");

		load_content(event_admin, resource_admin, settings_admin, world);
